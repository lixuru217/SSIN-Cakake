#!/usr/bin/env python3
"""
AES (CTR mode) implementation derived from MIRACL Core reference tables.
"""
...
PTAB = [
    ...
    122, 142, 137, 128, 155, 182, 193, 88, 232, 35, 101, 175, 234, 37, 111, 177, 200, 67, 197, 84, 252,
    31, 33, 99, 165, 244, 7, 9, 27, 45, 119, 153, 176, 203, 70, 202, 69, 207, 74, 222, 121, 139, 134,
    145, 168, 227, 62, 66, 198, 81, 243, 14, 18, 54, 90, 238, 41, 123, 141, 140, 143, 138, 133, 148,
...
LTAB = [
    ...
    244, 234, 214, 116, 79, 174, 233, 213, 231, 230, 173, 232, 44, 215, 117, 122, 235, 22, 11, 245, 89,
    203, 95, 176, 156, 169, 81, 160, 127, 12, 246, 111, 23, 196, 73, 236, 216, 67, 31, 45, 164, 118,
    123, 183, 204, 187, 62, 90, 251, 96, 177, 134, 59, 82, 161, 108, 170, 85, 41, 157, 151, 178, 135,
    144, 97, 190, 220, 252, 188, 149, 207, 205, 55, 63, 91, 209, 83, 57, 132, 60, 65, 162, 109, 71, 20,
    42, 158, 93, 86, 242, 211, 171, 68, 17, 146, 217, 35, 32, 46, 137, 180, 124, 184, 38, 119, 153, 227,
    165, 103, 74, 237, 222, 197, 49, 254, 24, 13, 99, 140, 128, 192, 247, 112, 7
]
...
FBSUB = [
    ...
    229, 241, 113, 216, 49, 21, 4, 199, 35, 195, 24, 150, 5, 154, 7, 18, 128, 226, 235, 39, 178, 117, 9,
    131, 44, 26, 27, 110, 90, 160, 82, 59, 214, 179, 41, 227, 47, 132, 83, 209, 0, 237, 32, 252, 177,
    91, 106, 203, 190, 57, 74, 76, 88, 207, 208, 239, 170, 251, 67, 77, 51, 133, 69, 249, 2, 127, 80,
    60, 159, 168, 81, 163, 64, 143, 146, 157, 56, 245, 188, 182, 218, 33, 16, 255, 243, 210, 205, 12,
    19, 236, 95, 151, 68, 23, 196, 167, 126, 61, 100, 93, 25, 115, 96, 129, 79, 220, 34, 42, 144, 136,
    70, 238, 184, 20, 222, 94, 11, 219, 224, 50, 58, 10, 73, 6, 36, 92, 194, 211, 172, 98, 145, 149,
...
RBSUB = [
    ...
    179, 69, 6, 208, 44, 30, 143, 202, 63, 15, 2, 193, 175, 189, 3, 1, 19, 138, 107, 58, 145, 17, 65,
    79, 103, 220, 234, 151, 242, 207, 206, 240, 180, 230, 115, 150, 172, 116, 34, 231, 173, 53, 133,
    226, 249, 55, 232, 28, 117, 223, 110, 71, 241, 26, 113, 29, 41, 197, 137, 111, 183, 98, 14, 170, 24,
    190, 27, 252, 86, 62, 75, 198, 210, 121, 32, 154, 219, 192, 254, 120, 205, 90, 244, 31, 221, 168,
    51, 136, 7, 199, 49, 177, 18, 16, 89, 39, 128, 236, 95, 96, 81, 127, 169, 25, 181, 74, 13, 45, 229,
...
def _rotl8(x: int) -> int:
    return _rotl(x, 8)


def _rotl16(x: int) -> int:
    return _rotl(x, 16)


def _rotl24(x: int) -> int:
    return _rotl(x, 24)


def _bmul(x: int, y: int) -> int:
    if x == 0 or y == 0:
        return 0
    lx = LTAB[x & 0xFF] & 0xFF
    ly = LTAB[y & 0xFF] & 0xFF
...
def _inv_mix_col(x: int) -> int:
    result = []
    m = _pack(IN_CO)
    for _ in range(4):
        result.append(_product(m, x))
        m = _rotl24(m)
    return _pack(result)
...
class MiraclAES:
    def __init__(self, key: bytes):
        ...
        self.nk = len(key) // 4
        self.nr = self.nk + 6
        self.fkey = [0] * (4 * (self.nr + 1))
        self.rkey = [0] * (4 * (self.nr + 1))
    ...
    def _expand_key(self, key: bytes) -> None:
        ...
        for i in range(nk):
            self.fkey[i] = _pack(key[4 * i : 4 * (i + 1)])

        j = nk
        k = 0
        while j < N:
            temp = self.fkey[j - 1]
            temp = _sub_word(_rotl24(temp))
            temp ^= RCO[k] & 0xFF
            self.fkey[j] = (self.fkey[j - nk] ^ temp) & 0xFFFFFFFF
            for i in range(1, nk):
                if j + i >= N:
                    break
                if nk > 6 and i == 4:
                    self.fkey[j + i] = (
                        self.fkey[j + i - nk] ^ _sub_word(self.fkey[j + i - 1])
                    ) & 0xFFFFFFFF
                else:
                    self.fkey[j + i] = (
                        self.fkey[j + i - nk] ^ self.fkey[j + i - 1]
                    ) & 0xFFFFFFFF
            j += nk
            k += 1
        ...
        for i in range(4, N - 4, 4):
            idx = N - 4 - i
            for j in range(4):
                self.rkey[idx + j] = _inv_mix_col(self.fkey[i + j])
    ...
    def encrypt_block(self, block: bytes) -> bytes:
        ...
        state = [_pack(block[i : i + 4]) for i in range(0, 16, 4)]
        for i in range(4):
            state[i] = (state[i] ^ self.fkey[i]) & 0xFFFFFFFF

        k = 4
        for _ in range(1, self.nr):
            t0 = (
                self.fkey[k]
                ^ FTABLE[state[0] & 0xFF]
                ^ _rotl8(FTABLE[(state[1] >> 8) & 0xFF])
                ^ _rotl16(FTABLE[(state[2] >> 16) & 0xFF])
                ^ _rotl24(FTABLE[(state[3] >> 24) & 0xFF])
            )
            t1 = (
                self.fkey[k + 1]
                ^ FTABLE[state[1] & 0xFF]
                ^ _rotl8(FTABLE[(state[2] >> 8) & 0xFF])
                ^ _rotl16(FTABLE[(state[3] >> 16) & 0xFF])
                ^ _rotl24(FTABLE[(state[0] >> 24) & 0xFF])
            )
            t2 = (
                self.fkey[k + 2]
                ^ FTABLE[state[2] & 0xFF]
                ^ _rotl8(FTABLE[(state[3] >> 8) & 0xFF])
                ^ _rotl16(FTABLE[(state[0] >> 16) & 0xFF])
                ^ _rotl24(FTABLE[(state[1] >> 24) & 0xFF])
            )
            t3 = (
                self.fkey[k + 3]
                ^ FTABLE[state[3] & 0xFF]
                ^ _rotl8(FTABLE[(state[0] >> 8) & 0xFF])
                ^ _rotl16(FTABLE[(state[1] >> 16) & 0xFF])
                ^ _rotl24(FTABLE[(state[2] >> 24) & 0xFF])
            )
            state = [
                t0 & 0xFFFFFFFF,
                t1 & 0xFFFFFFFF,
        ...
        result_words = [
            self.fkey[k]
            ^ (FBSUB[state[0] & 0xFF])
            ^ _rotl8(FBSUB[(state[1] >> 8) & 0xFF])
            ^ _rotl16(FBSUB[(state[2] >> 16) & 0xFF])
            ^ _rotl24(FBSUB[(state[3] >> 24) & 0xFF]),
            self.fkey[k + 1]
            ^ (FBSUB[state[1] & 0xFF])
            ^ _rotl8(FBSUB[(state[2] >> 8) & 0xFF])
            ^ _rotl16(FBSUB[(state[3] >> 16) & 0xFF])
            ^ _rotl24(FBSUB[(state[0] >> 24) & 0xFF]),
            self.fkey[k + 2]
            ^ (FBSUB[state[2] & 0xFF])
            ^ _rotl8(FBSUB[(state[3] >> 8) & 0xFF])
            ^ _rotl16(FBSUB[(state[0] >> 16) & 0xFF])
            ^ _rotl24(FBSUB[(state[1] >> 24) & 0xFF]),
            self.fkey[k + 3]
            ^ (FBSUB[state[3] & 0xFF])
            ^ _rotl8(FBSUB[(state[0] >> 8) & 0xFF])
            ^ _rotl16(FBSUB[(state[1] >> 16) & 0xFF])
            ^ _rotl24(FBSUB[(state[2] >> 24) & 0xFF]),
        ]

        output = bytearray(16)
        for idx, word in enumerate(result_words):
            for jdx, byte in enumerate(_unpack(word & 0xFFFFFFFF)):
                output[idx * 4 + jdx] = byte & 0xFF
        return bytes(output)

    def ctr_keystream(self, nonce: bytes) -> Iterable[bytes]:
        counter = bytearray(nonce)
        while True:
            yield self.encrypt_block(bytes(counter))
            for i in range(16):
                counter[i] = (counter[i] + 1) & 0xFF
                if counter[i] != 0:
                    break

    def ctr_crypt(self, data: bytes, nonce: bytes) -> bytes:
        if len(nonce) != 16:
            raise ValueError("nonce must be 16 bytes")
        stream = self.ctr_keystream(nonce)
...
class MiraclCTRChannel:
    def __init__(self, key: bytes):
        if len(key) not in (16, 24, 32):
            raise ValueError("channel key must be 16/24/32 bytes")
        self.key = key
        self.aes = MiraclAES(key)

    def encrypt(self, plaintext: bytes) -> EncryptedPayload:
        nonce = os.urandom(16)
        ciphertext = self.aes.ctr_crypt(plaintext, nonce)
        return EncryptedPayload(nonce, ciphertext)

    def decrypt(self, payload: EncryptedPayload) -> bytes:
        return self.aes.ctr_crypt(payload.ciphertext, payload.nonce)

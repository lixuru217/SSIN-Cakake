#!/usr/bin/env python3
"""
Implementation of the Zhu et al. (2023) conditional privacy-preserving
aggregate signature protocol for vehicular networks.

The module models the four high-level phases described in the scheme:

1. System setup handled by the trusted authority (combining the KGC and TRA)
2. Vehicle registration that issues a pseudo identity and partial secret key
3. Message signing performed by vehicles
4. Verification and aggregate verification performed by the RSU/application

The code mirrors the symbolic description to keep the mathematical structure
visible while remaining practical for simulations alongside the SSIN, Guo2021,
and Liu2022 implementations.
"""
...
def hash_H1(shared_point: bytes, t_pub: bytes, validity_ts: int) -> bytes:
    return hash_to_bytes(b"Zhu2023/H1", shared_point, t_pub, timestamp_to_bytes(validity_ts))


def hash_H2(p_pub: bytes, pseudo_id_encoded: bytes, B_i: bytes) -> int:
    return hash_to_scalar(b"Zhu2023/H2", p_pub, pseudo_id_encoded, B_i)


def hash_H3(p_pub: bytes, pseudo_id_encoded: bytes, B_i: bytes, ts: int) -> int:
    return hash_to_scalar(b"Zhu2023/H3", p_pub, pseudo_id_encoded, B_i, timestamp_to_bytes(ts))
...
class AggregateSignature:
    ...
    aggregate_scalar: int
    coefficients: List[int]
    U_values: List[bytes]

    def __post_init__(self) -> None:
        if len(self.coefficients) != len(self.U_values):
            raise ValueError("coefficient and U_i lengths mismatch")
...
class TrustedAuthority:
    """Combines the KGC and TRA roles for the Zhu2023 protocol."""
    ...
    def issue_pseudo_identity(self, request: PseudoIdRequest) -> PseudoIdResponse:
        id1_point = point_from_bytes(request.component1)
        shared_point = self._msk_tra * id1_point
        h1 = hash_H1(point_to_bytes(shared_point), self._params.tra_public_bytes, request.validity_ts)
    ...
    def issue_partial_secret(self, pseudo_id: PseudoIdentity) -> PartialSecretKey:
        ...
        h2 = hash_H2(self._params.kgc_public_bytes, pseudo_id.encode(), B_i_bytes)
    ...
    def trace_vehicle(self, pseudo_id: PseudoIdentity) -> Optional[str]:
        id1_point = point_from_bytes(pseudo_id.component1)
        shared_point = self._msk_tra * id1_point
        h1 = hash_H1(point_to_bytes(shared_point), self._params.tra_public_bytes, pseudo_id.expiry_ts)
...
class VehicleState:
    ...
    def sign(self, message: bytes | str, timestamp: int, randomness: Optional[int] = None) -> SignedBroadcast:
        ...
        h3 = hash_H3(self.parameters.kgc_public_bytes, pseudo_encoded, self.public_key.B_i, timestamp)
        h4 = hash_H4(message_bytes, self.parameters.kgc_public_bytes, pseudo_encoded, vpk_encoded, timestamp, U_i_bytes)

        delta = normalize_scalar(
            self.secret_key.c_i + self.secret_key.r_i * h3 + u_i * h4
        )
...
class RSUVerifier:
    ...
    SMALL_EXP_BOUND = 1 << 5  # 2^ell with ell = 5
    ...
    def verify(self, broadcast: SignedBroadcast, now: Optional[int] = None) -> bool:
        ...
        B_point = point_from_bytes(broadcast.public_key.B_i)
        if broadcast.timestamp > broadcast.pseudo_id.expiry_ts:
            return False

        h2 = hash_H2(self.parameters.kgc_public_bytes, pseudo_encoded, broadcast.public_key.B_i)
        h3 = hash_H3(self.parameters.kgc_public_bytes, pseudo_encoded, broadcast.public_key.B_i, broadcast.timestamp)
        h4 = hash_H4(
            broadcast.message,
            self.parameters.kgc_public_bytes,
            pseudo_encoded,
            broadcast.public_key.encode(),
            broadcast.timestamp,
            broadcast.signature.U_i,
        )

        lhs = scalar_mul_point(broadcast.signature.delta_i, self.parameters.generator_bytes)

        rhs_points: List[ECp] = [
            B_point,
            scalar_mul_point(h3, broadcast.public_key.R_i),
            scalar_mul_point(h2, self.parameters.kgc_public_bytes),
            scalar_mul_point(h4, broadcast.signature.U_i),
        ]
        rhs = add_points(rhs_points)

        return point_to_bytes(lhs) == point_to_bytes(rhs)

    def aggregate(self, broadcasts: Sequence[SignedBroadcast], coefficients: Optional[Sequence[int]] = None) -> AggregateSignature:
        if not broadcasts:
        ...
        if coefficients is None:
            coeffs = [1]
            for _ in range(1, n):
                coeffs.append(1 + secrets.randbelow(self.SMALL_EXP_BOUND))
        else:
            coeffs = list(coefficients)
            if len(coeffs) != n:
                raise ValueError("coefficient length mismatch")
            if coeffs[0] != 1:
                raise ValueError("first coefficient must be 1")
        if any(coeff <= 0 for coeff in coeffs):
            raise ValueError("coefficients must be positive")

        aggregate_scalar = 0
        for coeff, broadcast in zip(coeffs, broadcasts):
            aggregate_scalar = normalize_scalar(aggregate_scalar + coeff * broadcast.signature.delta_i)

        U_values = [broadcast.signature.U_i for broadcast in broadcasts]
        return AggregateSignature(aggregate_scalar=aggregate_scalar, coefficients=coeffs, U_values=U_values)
...
class ApplicationServer:
    ...
    def verify_aggregate(
        ...
        if not broadcasts:
            return False
        if len(broadcasts) != len(aggregate_signature.coefficients):
            return False
        if len(broadcasts) != len(aggregate_signature.U_values):
            return False

        coeffs = aggregate_signature.coefficients
        if coeffs[0] != 1:
            return False
        ...
        # Base signature (i = 1 in the paper)
        base = broadcasts[0]
        if base.timestamp > base.pseudo_id.expiry_ts:
            return False
        if now is not None and (base.timestamp > now or base.pseudo_id.expiry_ts < now):
            return False

        h2_base = hash_H2(self.parameters.kgc_public_bytes, pseudo_encoded[0], base.public_key.B_i)
        h3_base = hash_H3(self.parameters.kgc_public_bytes, pseudo_encoded[0], base.public_key.B_i, base.timestamp)
        h4_base = hash_H4(
            base.message,
            self.parameters.kgc_public_bytes,
            pseudo_encoded[0],
            base.public_key.encode(),
            base.timestamp,
            aggregate_signature.U_values[0],
        )

        if aggregate_signature.U_values[0] != base.signature.U_i:
            return False

        rhs_components.extend(
            [
                point_from_bytes(base.public_key.B_i),
                scalar_mul_point(h3_base, base.public_key.R_i),
                scalar_mul_point(h2_base, self.parameters.kgc_public_bytes),
                scalar_mul_point(h4_base, aggregate_signature.U_values[0]),
            ]
        )

        for idx in range(1, len(broadcasts)):
            broadcast = broadcasts[idx]
            if broadcast.timestamp > broadcast.pseudo_id.expiry_ts:
                return False
            if now is not None and (broadcast.timestamp > now or broadcast.pseudo_id.expiry_ts < now):
                return False
            if aggregate_signature.U_values[idx] != broadcast.signature.U_i:
                return False

            coeff = coeffs[idx]
            encoded = pseudo_encoded[idx]

            h2_i = hash_H2(self.parameters.kgc_public_bytes, encoded, broadcast.public_key.B_i)
            h3_i = hash_H3(self.parameters.kgc_public_bytes, encoded, broadcast.public_key.B_i, broadcast.timestamp)
            h4_i = hash_H4(
                broadcast.message,
                self.parameters.kgc_public_bytes,
                encoded,
                broadcast.public_key.encode(),
                broadcast.timestamp,
                aggregate_signature.U_values[idx],
            )

            rhs_components.append(scalar_mul_point(coeff, broadcast.public_key.B_i))
            rhs_components.append(scalar_mul_point(normalize_scalar(coeff * h3_i), broadcast.public_key.R_i))
            rhs_components.append(scalar_mul_point(normalize_scalar(coeff * h4_i), aggregate_signature.U_values[idx]))
            sum_h2 = normalize_scalar(sum_h2 + coeff * h2_i)

        rhs_components.append(scalar_mul_point(sum_h2, self.parameters.kgc_public_bytes))

        rhs = add_points(rhs_components)
        lhs = scalar_mul_point(aggregate_signature.aggregate_scalar, self.parameters.generator_bytes)
        return point_to_bytes(lhs) == point_to_bytes(rhs)

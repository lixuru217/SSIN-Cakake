#!/usr/bin/env python3
"""Simple Zhu2023 vehicular network simulation."""
...
import zhu2023  # type: ignore  # noqa: E402  pylint: disable=wrong-import-position


def _init_vehicle(ta: zhu2023.TrustedAuthority, real_identity: str, validity_ts: int) -> zhu2023.VehicleState:
    params = ta.public_parameters()
    vehicle = zhu2023.VehicleState(real_identity, params)
...
def _run_round(
    rsu: zhu2023.RSUVerifier,
    app: zhu2023.ApplicationServer,
    vehicles: Sequence[zhu2023.VehicleState],
    *,
    timestamp: int,
    base_message: str,
    noisy: bool,
) -> int:
    broadcasts: List[zhu2023.SignedBroadcast] = []
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Zhu2023 vehicular signing simulation.")

#!/usr/bin/env python3
"""Target RSU (LEO2) server for Zhu2023 handover experiments."""
...
from zhu2023_codec import encode_switch_response, broadcast_from_dict, decode_payload, ensure_type
from zhu2023_context import get_parameters, load_context


def _load_parameters(context_path: Path) -> Tuple[zhu2023.PublicParameters, float]:
    context = load_context(context_path)
    params = get_parameters(context)
    mtime = context_path.stat().st_mtime_ns
    return params, float(mtime)

logger = logging.getLogger("zhu2023_rsu2")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Zhu2023 target RSU server")
...
def main() -> None:
    ...
    verifier = zhu2023.RSUVerifier(params)

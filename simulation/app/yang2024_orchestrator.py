#!/usr/bin/env python3
"""Scenario runner for Yang2024 single-link authentication experiments."""
...
DEFAULT_CONTEXT_HOST_PATH = Path(__file__).resolve().parents[1] / "shared" / "offline_context_yang2024.pkl"
DEFAULT_CONTEXT_CONTAINER_PATH = Path("/shared/offline_context_yang2024.pkl")
...
def ensure_containers_running(containers: Iterable[str]) -> None:
    ...
    if missing:
        raise RuntimeError(
            ...
            + "'docker compose -f simulation/docker-compose-yang2024.yml up -d'."
        )
...
def invoke_ue_client(
    *,
    ...
    try:
        ...
        emit_container_output("YANG-UE", stdout, stderr)
    ...
    emit_container_output("YANG-UE", proc.stdout, proc.stderr)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Yang2024 single-link orchestrator")
    ...
    parser.add_argument("--output", type=Path, default=Path("results_yang2024.csv"), help="CSV output path")
